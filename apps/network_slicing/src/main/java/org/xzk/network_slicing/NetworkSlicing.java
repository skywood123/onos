...
public class NetworkSlicing implements netinfo{
    ...
    //Disable for compile
   // @Reference(cardinality = ReferenceCardinality.MANDATORY)
   // protected p4meterservice meterservice;
    ...
    public void getinfo(){
    HashMap<NetworkId, HashMap<FlowPair, List<FlowRuleInformation>>> example=flowRuleStorage.getAllFlows();
    for (NetworkId name: example.keySet()){
            String key = name.toString();
	   System.out.println("flowpair size = " + example.get(name).keySet().size());
	    for(FlowPair pair:example.get(name).keySet()){

            String keypair=pair.toString();
            System.out.println("flowpair src=  " + pair.getSrc().toString() + " flowpair dst= "+pair.getDst().toString());

            List<FlowRuleInformation> rulelist=example.get(name).get(pair);
            System.out.println("printing flowruleinformation for flowpair " + keypair);
	    for(FlowRuleInformation info: rulelist){
		System.out.println("Flowrule : " + info.getFlowRule().toString());
		if(info.getMplsLabel()!=null)
                System.out.println("mplslabel = " + info.getMplsLabel().toString());
